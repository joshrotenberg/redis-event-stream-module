...
fn entry_format_is_runtime_mutable_producing_a_mixed_stream() {
    ...
    wait_until(CAPTURE_WAIT, "first entry", || {
        xlen(&mut c, "events:set") == 1
    });
    ...
    wait_until(CAPTURE_WAIT, "second entry", || {
...
fn seq_resets_on_reload() {
    ...
    wait_until(CAPTURE_WAIT, "two events mirrored", || {
        xlen(&mut c, "events:set") == 2
    });
    ...
    let mut c = s.conn();
    wait_until(CAPTURE_WAIT, "aof reloaded", || {
        xlen(&mut c, "events:set") == 2
    });
    let _: () = c.set("cc", "3").expect("SET after restart");
    wait_until(CAPTURE_WAIT, "third event mirrored", || {
