...
On a healthy capturing master, each selected event produces exactly one
canonical per-event stream entry, atomically with the keyspace change. Overall
capture is at-most-once because events cannot be recovered when the module is
unloaded, disabled, unable to write, or lost with unpersisted Redis state.
Consumer groups provide at-least-once processing for entries that remain
inside the retention window, so handlers must be idempotent.
