...
- Cluster support: the slot-pinned per-node hashtag design shipped in v0.2.0 as `eventstream.cluster-streams per-node` (section 10), including per-node discovery and reshard handling. The as-built mechanism is documented in [docs/cluster-support.md](docs/cluster-support.md) (rewritten from the original proposal, issue #83), with the rejected alternatives and design history retained in [docs/cluster-design-history.md](docs/cluster-design-history.md).
