...
| First failure per drop reason (`dropped_xadd_error`, `dropped_oom`, `dropped_defer_error`, `dropped_migrating`, `dropped_no_owned_slot`): the failure detail — server error text, or a fixed description for `dropped_no_owned_slot`, which has none | warning |
...
Per-stream rate-limited logging with recovery notices (one warning per stream per 60 seconds, suppressed-count summaries) is Future work (issue #68); the counters never lose data even when the log says nothing.
...
- Per-stream rate-limited failure logging with recovery notices (issue #68); custom `@eventstream` ACL category (needs `RM_AddACLCategory`, Redis 7.4+, with 7.2/7.3 fallback) (issue #69).
