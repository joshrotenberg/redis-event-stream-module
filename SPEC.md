...
`EVENTSTREAM.STREAMS` reads a set that lives in database 0; the command selects database 0 for the read and restores the caller's database before returning. Both commands touch only keys under the prefix (or no keys), so a least-privilege consumer ACL that already grants `~<prefix>*` covers them; grant the module's commands explicitly if the ACL restricts by command.

Both commands are tagged into a custom `@eventstream` ACL category, so a command-restricted ACL can grant or deny the module's commands as a group with `+@eventstream` (or `-@eventstream`) instead of naming each one, and future module commands join the category automatically. The category needs `RM_AddACLCategory` (Redis 7.4+); it registers on Redis 7.4/8 and Valkey builds that expose the API, and is skipped without failing the load on Redis 7.2/7.3 (the module logs one notice at load and the commands stay individually grantable as `+eventstream.stats +eventstream.streams`). Use `ACL CAT` to confirm the category exists on a given server before relying on it. The category is additive: the flag-derived `@read` and `@fast` categories still apply everywhere.
