...
| Hash-field expiration (7.4+) | `hexpired`, plus command-generated `hexpire`, `hpersist` | Fire under the HASH class, not EXPIRED, so the default `expired` filter does not match `hexpired` (section 7). `hexpired` has the same lazy/active timing as `expired` and carries only the hash key, not the field name (section 6). When the last field expires the emptied hash is deleted and a `del` event fires. Absent on servers without hash-field TTLs (Redis before 7.4). |
...
Discovery has two paths. The `EVENTSTREAM.STREAMS` command (section 8) returns every destination stream registered since the registry existed, read live from a persistent set at `<prefix>#streams`. The set is SADD-ed on the first write to each stream through the same replicated, OOM-checked call as the mirrored entry, so it survives restart under RDB or AOF and is present on replicas; an in-process cache suppresses the SADD on subsequent writes and is cleared on flush so a `FLUSHALL` that deleted the set rebuilds it on the next capture. Registration is a best-effort side effect, separate from capturing the event: if the SADD fails while the mirrored `XADD` succeeded (a foreign non-set key parked at `<prefix>#streams` giving `WRONGTYPE`, an OOM on the set write), the event is still captured — it is not a lost event and not a failed *destination* write — but the stream is not yet discoverable through `EVENTSTREAM.STREAMS` (indeed, a wrong-type registry key makes that command itself error until the key is fixed). The failure is observable in `registry_errors` (section 13) with a first-failure log, the in-process cache stays unregistered so the next write to the stream retries, and a recovery notice logs when it succeeds (issue #216). The cap counts the stream regardless (section 7), so registration health never affects `max-streams`. The registry is an append-only log of stream names ever written, not a liveness check: a listed stream may since have been trimmed to empty or deleted.

`EVENTSTREAM.STREAMS VERBOSE` annotates that history against the live keyspace (issue #81): per registered name it returns existence and current `XLEN` (`[name, exists, length]`), so a consumer gets a trustworthy discovery answer — which registered streams still hold entries — in one round-trip instead of a client-side `XLEN` join per name. `exists` (`EXISTS`, `0`/`1`) and `length` (`XLEN`) are independent probes, so a present but empty or foreign non-stream key reads `exists 1, length 0`. It is read-only and mutates nothing; the append-only semantics are unchanged, and like the bare and `WITHSTATS` forms it runs on replicas.

Reconciling the registry itself is the separate `EVENTSTREAM.PRUNE` command (section 8) — the only path that mutates the registry, and never automatic. It removes (`SREM`) the registered names whose destination key is **absent** (`EXISTS 0`: deleted) and returns the count removed. Absence read via `EXISTS` is the sole prune trigger: a present key that is empty, or a foreign non-stream key parked at the name (whose `XLEN` would error), is not absent and is never pruned. The `SREM` replicates like the registration `SADD` so replicas and the AOF/RDB converge, but without the verify-oom flag — pruning frees memory, so gating it under `maxmemory` would be wrong. Each pruned name's in-process dedupe is invalidated in the same operation so a later write re-registers it (the append-only default is otherwise intact). Because the existence check and the `SREM` run within one command execution, a stream recreated between them is not pruned on the non-cluster node. `EVENTSTREAM.PRUNE` is a `write` command; keeping it a distinct command is what lets `EVENTSTREAM.STREAMS` stay `readonly` and serve discovery on replicas.
...
- **Per source node in cluster mode.** Each node's module instance has its own counter, so `seq` orders entries per node, not across nodes. Cross-node same-millisecond ties still need an application timestamp (section 9, docs/cluster-consumers.md).
...
| `eventstream.maxlen-overrides` | string | `` (empty) | yes | comma-separated `event=cap` pairs (grammar below) overriding `maxlen` per destination stream; each cap is `0` (trimming disabled for that stream) or positive. Same module-arg boundary-bypass quirk as `maxlen`, so caps are re-validated and a negative rejects the set / aborts the load. Empty means no overrides |
| `eventstream.retention-ms` | i64 | `0` | yes | `0` (disabled) to `i64::MAX`; when `> 0`, streams trim by time (`XADD ... MINID ~ <now-ms − retention-ms>`) instead of by count, taking precedence over `maxlen` (below). Same module-arg boundary-bypass quirk as `maxlen`, so the binding re-validates and rejects negatives, aborting the load |
...
| `eventstream.verify-oom` | bool | `yes` | yes | `yes` / `no`; when `no`, mirrored writes drop the `M` flag and are not refused under `maxmemory` (section 11 trade-off). Default `yes` keeps the bounded-counted-loss behavior |
...
**`eventstream.maxlen-overrides`.** Per-event `maxlen` caps (issue #62): a comma-separated list of `event=cap` pairs (grammar below) keyed by the destination stream suffix, overriding the global `eventstream.maxlen` for the named streams. Event rates differ wildly across event types, and the sizing formula (`maxlen ≥ peak_event_rate × worst_case_consumer_downtime`, section 11) is per-stream; with one global cap the worst-case stream dictates every stream's size. An operator who needs 600k `expired` entries pays that on `set`, `del`, and every other name too. Overrides break that coupling: at each inline `XADD ... MAXLEN ~` site the effective cap is the override for that stream if present, else the global `maxlen`. The default (empty) resolves every stream to the global cap, so behavior is unchanged until an operator opts in. The `#control` stream suffix is addressable as an override key (the `#` namespace cannot collide with a sanitized event name); the firehose is deliberately **not** override-addressable and uses the global `maxlen`, because it aggregates every event type and is sized for the total rate (section 11). The parsed map is stored behind the same `RedisGILGuard` discipline as `eventstream.events`; the write path reads it under the GIL and resolves the per-stream cap when the suffix is final. When `eventstream.retention-ms` is set it takes precedence and overrides are ignored (time-based trimming replaces the `MAXLEN` clause entirely). The override map's entry count appears in the `loaded` log line; the full map is available through `CONFIG GET eventstream.maxlen-overrides`.

**`eventstream.retention-ms`.** Time-based retention (issue #108): when `> 0`, mirrored writes trim by `XADD ... MINID ~ <now_ms − retention_ms>` instead of `MAXLEN`, dropping entries older than the window. Operational retention is usually expressed in time ("keep 24h") rather than entry counts, and under bursty traffic a fixed `maxlen` gives an unpredictable replay window — a burst can flush hours of history in seconds. Because destination streams use auto IDs (`*`), every entry ID already carries the event's millisecond timestamp (from `RedisModule_Milliseconds`, the same server clock that stamps the IDs), so `MINID` implements wall-clock retention with no extra bookkeeping. It stays inline, folded into the `XADD`, for the same reason `maxlen` rejected periodic `XTRIM` (above): no extra writes, no timer. Two consequences: (1) Redis accepts either `MAXLEN` or `MINID` in one `XADD`, not both, so the two are mutually exclusive — when `retention-ms > 0` it wins and `maxlen` (and any per-event override) is ignored for the trim clause; `set()` logs a notice at the config-change point so the ignored cap is not a silent surprise. (2) Inline-only trimming means an idle stream that stops receiving events is never re-trimmed and its entries can persist past the window; this mirrors the documented `maxlen` idle-stream behavior (live-change table below) and is a caveat, not a bug — external periodic `XTRIM MINID` remains the mitigation. `0` disables it, preserving count-based-only behavior. Approximate (`~`) trimming, matching the memory-model rationale (section 11).

**`eventstream.verify-oom`.** Whether mirrored writes respect `maxmemory` (issue #65). Default `yes` keeps the `M` flag on the `call_ext` options (section 10), so an `XADD` is refused under `maxmemory` and counted as `dropped_oom` — the deliberate bounded-counted-loss default (section 11: forcing writes past `maxmemory` grows memory exactly when the server is shrinking it). Set `no` and the module drops the `M` flag, so capture continues at the memory limit. The trade-off (documented in section 11): growth stays bounded by `maxlen`, but the module now adds memory while the server is evicting, which can accelerate eviction of user keys and amplifies the `evicted`-storm case. Some deployments — where the `expired`/`evicted` record is the whole point of the module — prefer that to losing the pressure window. Read per `XADD`, so a live `CONFIG SET` needs no cached-options invalidation and applies to mirrored entries, gap markers, and firehose copies alike; with the flag off the `dropped_oom` counter and `OOM` error branches stay in place but never fire.

**`eventstream.max-streams`.** Caps the number of distinct destination streams the module will create. Module-defined event names are the only unbounded source of streams (section 5): a wide-open filter against a buggy or hostile co-loaded module grows memory linearly with distinct names (`total ≈ distinct_event_names × maxlen × bytes_per_entry`, section 11), and neither the 128-byte name cap nor per-stream `maxlen` bounds the *count*. The default `0` is unlimited. When the cap is nonzero and a captured event would create a new stream beyond it, the event is dropped and counted (`dropped_max_streams`, part of the `dropped` sum) rather than captured; streams already registered keep receiving events. The count backing the cap is process-local and tracks currently created streams: it counts a stream on its first successful `XADD` regardless of whether the registry `SADD` succeeds (issue #216), so the cap bounds created destination streams, not registrations. It resets on flush and on restart (so the first `max-streams` distinct names seen afterward win, and the persistent registry stops growing once the in-process count re-fills), and `EVENTSTREAM.PRUNE` decrements it as it removes dead names (section 8). It matches `active_streams` in unit (both count created streams) but resets on flush where `active_streams` is the since-load lifetime count that never resets (section 13). Lowering the cap below the current count at runtime is accepted: existing streams continue and no new streams are created until the count is under the cap again. The control stream and gap markers are exempt (the `#` namespace is not event-derived and markers must never be dropped); the firehose, being a single fixed `#`-namespaced stream, is likewise never itself blocked, but it does count like any destination stream and so occupies one cap slot (matching `active_streams`, which counts it) — size `max-streams` as one larger when the firehose is enabled. Enforced at the point a first write creates a new stream (section 5 Discovery), checked before the `XADD` and committed when it succeeds.
...
### Maxlen-overrides grammar

`eventstream.maxlen-overrides` (issue #62) is empty (no overrides) or a comma-separated list of `event=cap` pairs:

```
maxlen-overrides := "" | pair ( "," pair )*
pair             := event-suffix "=" cap
event-suffix     := destination stream suffix (sanitized event name, or #control)
cap              := non-negative decimal integer (0 disables trimming for that stream)
```

- Whitespace around each pair and around the `=` is trimmed; the empty string is the default (empty map).
- The key is matched against the destination stream **suffix**, i.e. the sanitized event name (section 5); for built-in and known event names the suffix is byte-identical to the event name, so `expired=600000,set=1000` sizes `events:expired` and `events:set`. A literal `#control` targets the control stream (section 9); the firehose is not addressable (it uses the global `maxlen`).
- Each cap is validated `0` or positive, re-validated in `set()` because module-arg values bypass the registered range (same mechanics as `maxlen`). A pair with no `=`, an empty event name, a non-integer cap, a negative cap, or an empty token rejects the whole value at `CONFIG SET` time (or aborts the load for a module arg), matching `eventstream.events` parse-failure behavior.
- A caller reads the effective cap for any stream as: override for that suffix if present, else `eventstream.maxlen`. When `eventstream.retention-ms > 0`, overrides (and `maxlen`) are ignored — time-based trimming replaces the count-based clause.

| Value | Effect |
|---|---|
| `` (empty, default) | every stream trims under the global `maxlen` |
| `expired=600000` | `events:expired` retains 600k; all other streams use the global cap |
...
| `maxlen-overrides` | new per-event caps apply on each `XADD` | old caps; an idle stream is re-trimmed only on its next write, as with `maxlen` |
| `retention-ms` | new window applies on each `XADD` (MINID); overrides `maxlen` when nonzero | old window; an idle stream is re-trimmed only on its next write, so entries can outlive the window until then |
| `verify-oom` | `M` flag added/dropped on each `XADD` from the next event | old flag (the write already completed) |
...
| `EVENTSTREAM.STREAMS [WITHSTATS \| VERBOSE]` | The registered destination streams (section 5 Discovery), read live from `<prefix>#streams`; per-argument reply below. A captured stream whose registry `SADD` failed is absent until a later write registers it (`registry_errors`, sections 5 and 13); a foreign non-set key at `<prefix>#streams` makes the command error `WRONGTYPE` | `readonly`, keyless (O(N) in registered streams, so not `fast`) |
...
`EVENTSTREAM.STREAMS` and `EVENTSTREAM.PRUNE` both read a set that lives in database 0; each selects database 0 for its reads (and, for `PRUNE`, the reconciling `SREM`s) and restores the caller's database before returning. `EVENTSTREAM.STREAMS` is `readonly` and runs on replicas — discovery surviving to a replica is the whole point; it never mutates the registry, and an unknown argument (including `PRUNE`, which is its own command now) is rejected. `EVENTSTREAM.PRUNE` is the only command that writes, so it — and only it — is flagged `write` and routed to the primary, which keeps `EVENTSTREAM.STREAMS` off the write path. All three commands touch only keys under the prefix (or no keys), so a least-privilege consumer ACL that already grants `~<prefix>*` covers them; grant the `eventstream.*` commands explicitly if the ACL restricts by command.

All three commands are tagged into a custom `@eventstream` ACL category, so a command-restricted ACL can grant or deny the module's commands as a group with `+@eventstream` (or `-@eventstream`) instead of naming each one, and future module commands join the category automatically. The category needs `RM_AddACLCategory` (Redis 7.4+); it registers where the API is present and is skipped without failing the load on Redis 7.2/7.3 (the module logs one notice at load and the commands stay individually grantable as `+eventstream.stats +eventstream.streams +eventstream.prune`). The module registers the category and tags the commands into it itself, during `init`, rather than through the wrapper's declarative path, because Redis keeps a module's ACL categories across `MODULE UNLOAD`: re-adding the category on an in-place reload (a second `MODULE LOAD` in the same process) would otherwise fail the load. Registering it directly lets the module tolerate the already-present category and re-tag the commands, so an in-place upgrade loads cleanly (issue #107). Use `ACL CAT` to confirm the category exists on a given server before relying on it. The category is additive: the flag-derived `@read`, `@fast`, and `@write` categories still apply everywhere.

The bare `EVENTSTREAM.STREAMS` reply is the flat array of stream names, unchanged. An argument selects one of two read-only annotated forms:

- `WITHSTATS` returns one array per registered stream: `[name, "forwarded", <n>, "dropped", <n>]` — the entries this process wrote to that stream and the entries a refused `XADD` to it dropped (the `dropped_xadd_error` and `dropped_oom` scopes; drops with no destination stream in hand, like defer errors, have no stream to land on). The counters are process-local: reset on load and cleared with the flush invalidation that clears the registry cache (per-stream counts therefore read "since load or last flush", while the section 13 process-wide counters remain strictly since-load; after a flush the two can disagree by the pre-flush writes). The registry itself is append-only across restarts, so a registered stream with no writes since load reports zeros. The firehose stream is a registered destination stream: its row's `forwarded` is the per-stream view of `firehose_forwarded`. The control stream is not in the registry and is not listed; a stream whose every write failed is likewise absent until its first successful write registers it (its drops are counted, but attributed rows exist only for registered streams).
- `VERBOSE` (issue #81) returns one array per registered stream: `[name, exists, length]`, where `exists` is `0`/`1` (`EXISTS` on the destination key) and `length` is its current `XLEN` (`0` for a missing key, an empty stream, or a foreign non-stream key whose `XLEN` errors). `exists` and `length` are independent probes: an absent key reads `0, 0`; a present-but-empty or wrong-type key reads `1, 0`. This annotates discovery output with liveness in a single round-trip without mutating the registry; the append-only history is unchanged.
...
5. An entry that is delivered fine but fails processing on every attempt (a "poison" entry) is the consumer's responsibility, not the module's: the module writes an entry once and never participates in the consumer group, so it has no delivery-count to act on. Bound the retries with the delivery counter (`XPENDING`/`XAUTOCLAIM`) and move the entry to an application-owned dead-letter stream — outside `<stream-prefix>` — after N attempts. The pattern is documented in the consumer guide (docs/work-queues.md, "Dead-lettering poison entries").
...
**Ordering footgun, and `eventstream.auto-group`.** The recipe above is race-free against first capture *when consumers deploy before the module*: `MKSTREAM` creates an empty stream and the group at `$` sees every subsequent entry. But when the module deploys first (the common order: enable capture, then roll out workers), the stream already has entries by the time the recipe runs, and a group created at `$` silently misses the head. The documented fix is to create the group at `0` instead of `$` (see Replay below) — correct, but it requires the operator to know which ordering they are in. `eventstream.auto-group <name>` (section 7) removes the footgun: the module creates the group at `0` on each destination stream at stream birth, so the group exists from the first entry regardless of deployment order, and workers can `XREADGROUP GROUP <name> ... 0` (drain) then `>` (tail) with no `XGROUP CREATE` step at all. It is off by default, applies to per-event streams and the firehose but not the control stream, replicates and persists like a mirrored entry, and is idempotent (`BUSYGROUP` is success). It does **not** change the slow-consumer contract above: a group at `0` still loses entries trimmed by `maxlen` before consumers catch up (point 4) — the guarantee is operational (the group exists from birth), not stronger delivery.
...
CallOptionsBuilder::new()
    ...
    .verify_oom()         // 'M' : respect maxmemory (conditional, below)
    .build()
```

The `M` flag is conditional on `eventstream.verify-oom` (section 7, issue #65): with the default `yes` it is present and an `XADD` is refused under `maxmemory`; with `no` the flag is dropped and mirrored writes proceed at the limit (section 11 trade-off). The options are rebuilt per `XADD` from the config atomic, so a live change needs no invalidation and applies uniformly to mirrored entries, gap markers, and firehose copies. Plain `ctx.call` uses format `"v"` only and does not replicate or reach the AOF (`src/raw.rs`), so it is never used for keyspace writes. Redis rewrites `XADD <key> ... *` to the concrete generated ID before propagation, so entry IDs are identical on master, replicas, and after AOF replay.
...
Eviction warning: `allkeys-*` policies can evict the event streams themselves. Recommend `noeviction` or `volatile-*` on instances running this module. The module surfaces this as the `eventstream_eviction_risk` INFO field (0/1, section 13), recomputed at load and on every config change, and logs a warning naming the policy when the risk is present; alert on the field flipping to 1.
...
- Eviction pressure: `evicted` fires while the server is trying to free memory. The `M` flag (`verify_oom`) makes the module drop and count rather than force `XADD`s past `maxmemory`, which would grow memory exactly when the server is shrinking it. Bounded, counted loss wins — the default. Opt-out (issue #65): `eventstream.verify-oom no` drops the `M` flag so capture continues at the limit. Growth stays bounded by `maxlen`, but the module then adds memory while the server is evicting, which can accelerate eviction of user keys and amplifies this `evicted`-storm case; choose it only where the capture record matters more than the transient memory pressure, and alert on `used_memory` instead of `dropped_oom` (which never fires while the flag is off).
...
Per-event overrides change the bound's shape (issue #62): with `eventstream.maxlen-overrides` the total is a sum of per-stream caps, `Σ stream_cap × bytes_per_entry` (each overridden stream at its cap, the rest at the global `maxlen`), rather than one cap times the stream count. This is the point — sizing the hot stream large no longer inflates every quiet stream.

Time-based retention changes it again (issue #108): with `eventstream.retention-ms` the per-stream bound is `event_rate × window × bytes_per_entry` — memory scales with the actual event rate over the window, not a fixed count. A quiet stream costs little; a burst is bounded by how much fits in the window, not by a count guessed in advance. `MINID` takes precedence over `maxlen`, so the count-based bound does not also apply; size for the peak rate over `retention-ms`. The idle-stream caveat (section 7) means a stopped stream can hold its last window's entries indefinitely until its next write.
...
| `maxmemory` reached | `XADD` refused via `M` flag, entry dropped | `dropped_oom` | Raise `maxmemory`, lower `maxlen`, or narrow the filter; or set `eventstream.verify-oom no` to continue capturing at the limit (section 11 trade-off: the module then adds memory during eviction) |
...
| Server below 7.2 | Module load fails; process abort at startup, not a clean refusal (the wrapper aborts before `init`'s version and `RM_AddPostNotificationJob`-pointer gates run, section 14) | n/a | Upgrade; see section 14 |
...
The module's own writes run server-side with module privileges and are not subject to any client's ACL: a user with no access to `events:*` can still cause writes to those keys by touching watched keys. That is by design (a server-level facility), documented for security review. Consumers need explicit grants, for example `ACL SETUSER consumer on >pw ~events:* +xread +xreadgroup +xack +xautoclaim +xinfo +xlen`. For the module's own commands, a strictly read-only consumer grants the two introspection commands by name (`+eventstream.stats +eventstream.streams`), which works on every supported version. `+@eventstream` (Redis 7.4+) grants the module's commands as a group instead, but it includes the `write` command `EVENTSTREAM.PRUNE` (keyless, so key patterns do not constrain it; its write is limited to registry reconciliation, section 8) and future module commands automatically. On Redis 7.2/7.3 the category is unavailable and name grants are the only form (section 8).
...
eventstream_eviction_risk:0
eventstream_forwarded:48211
eventstream_events_lost:0
eventstream_firehose_forwarded:0
eventstream_autogroup_created:0
eventstream_autogroup_failed:0
...
|---|---|---|
| `eventstream_events_lost` | INFO | any increase: a selected event produced no canonical entry (the total-loss SLO, issue #218). Includes a zero-slot master's complete loss, which `eventstream_dropped` omits |
| `eventstream_dropped` | INFO | any increase: a destination write failed (per write, includes firehose copies; see `events_lost` for lost events) |
...
| `eventstream_dropped_no_owned_slot` | INFO | any increase: a per-node master owns no slot and captured nothing (section 10); folded into `events_lost`, so alerting on `events_lost` covers it |
| `eventstream_enabled` | INFO | 0 when expected 1 |
| `eventstream_eviction_risk` | INFO | `1`: `maxmemory-policy` is `allkeys-*` and can evict the destination streams themselves (section 11); set `noeviction` or a `volatile-*` policy |
...
| Event | Level |
|---|---|
| Module loaded: effective config (prefix, filter, maxlen, maxlen-overrides count, retention-ms, verify-oom) | notice |
...
| First failure per drop reason with no destination stream in hand (`dropped_defer_error`, `dropped_migrating`, `dropped_no_owned_slot`, `dropped_encode_error`, `autogroup_failed`, `SelectDb(0)` failures): once per process | warning |
...
The crate builds with the wrapper's `min-redis-compatibility-version-7-2` feature. Under it, the macro-generated registration path (commands, then configs) unwraps 7.2-only API pointers before `init` ever runs, so on any pre-7.2 server the load panics inside the wrapper and aborts the redis-server process; with `loadmodule` in redis.conf that is a startup abort with the panic in the log, not the polite error message a clean refusal would give (verified against the wrapper source at v2.1.3). `init` carries two defense-in-depth gates that would fire — logging the requirement and returning `Status::Err` — if the wrapper's registration behavior ever becomes graceful: an explicit `ctx.get_redis_version()` check (extracted into the pure, unit-tested `version_supported` so the 7.2 boundary is pinned without spawning an old server, issue #77) and a direct null check of the `RedisModule_AddPostNotificationJob` pointer, which is null on any server predating it — the same null-pointer signal the 7.4+ canonical-name API gives in `select_owned_tag`, and a check that does not depend on parsing the reported version string. Both gates document the requirement in code; today, on a real pre-7.2 server, neither is reached because the wrapper aborts first. Alternatives rejected: writing inside the callback on older servers (documented unsafe, loses atomicity) and buffering through a `DetachedContext` background thread (loses atomicity, can drop on crash, adds GIL contention).
...
- `hexpired` routes under an explicit bare name and under `@hash`; the default `expired` filter does not match it (counted `skipped_filtered`); the `del` accompanying the last field's expiry is captured. Capability-gated on `HEXPIRE` presence, not server version. Portability caveat: the notification class `hexpired` carries can differ across server implementations, so `@hash` may not select it everywhere, while the explicit `hexpired` bare name matches regardless. Filter by name, not class, for portable hash-field-expiry capture.
...
- `EVENTSTREAM.STATS`, `EVENTSTREAM.STREAMS`, and `EVENTSTREAM.PRUNE` commands are implemented (section 8; keyless as planned. `STATS` is `readonly fast`; `STREAMS` is `readonly`, O(N) in registered streams so not flagged `fast`, and stays `readonly` so its `WITHSTATS`/`VERBOSE` discovery works on replicas; the opt-in registry cleanup is the separate `write` command `EVENTSTREAM.PRUNE`, issue #81).
...
- Runtime-mutable `stream-prefix`, with the current-plus-previous-prefix guard and documented old-stream cleanup semantics: considered and not planned (issue #59, closed 2026-07-14; section 17 Q3). Retained here as pre-scoping should the need ever become concrete.
...
- Cluster support: the slot-pinned per-node hashtag design shipped in v0.2.0 as `eventstream.cluster-streams per-node` (section 10), including per-node discovery and reshard handling. The as-built mechanism is documented in [docs/cluster-support.md](https://github.com/joshrotenberg/redis-event-stream-module/blob/main/docs/cluster-support.md) (rewritten from the original proposal, issue #83), with the rejected alternatives and design history retained in [docs/cluster-design-history.md](https://github.com/joshrotenberg/redis-event-stream-module/blob/main/docs/cluster-design-history.md).
- Key-name glob filter (`eventstream.key-filter`, issue #61), source-db filter (`eventstream.source-dbs`, issue #63), and a max-streams cap on distinct event names (`eventstream.max-streams`, issue #64) are implemented (section 7; counters `skipped_key_filtered`, `skipped_db`, and `dropped_max_streams`, section 13). The global monotonic `seq` entry field for cross-stream same-millisecond ordering is implemented (`eventstream.entry-seq`, sections 6 and 9, issue #66): an IMMUTABLE bool, default off, per-node/per-process. Per-event `maxlen` overrides (`eventstream.maxlen-overrides`, issue #62) and the option to disable `verify_oom` (`eventstream.verify-oom`, issue #65) are implemented (section 7), both runtime-mutable and default-preserving. Time-based retention via `MINID` trimming (`eventstream.retention-ms`, issue #108) is implemented alongside them (section 7): runtime-mutable, default `0` (disabled), taking precedence over `maxlen` when set — the count-based and time-based surfaces compose as designed.
...
- Custom `@eventstream` ACL category is implemented (section 8, issue #69): all three commands (`EVENTSTREAM.STATS`, `EVENTSTREAM.STREAMS`, and the later `EVENTSTREAM.PRUNE`) are tagged into it via `RM_AddACLCategory`/`RM_SetCommandACLCategories` where the API exists (Redis 7.4+), and it degrades to a clean skip with one load-time notice on Redis 7.2/7.3 (the null-pointer gate) — the commands stay individually grantable everywhere. Per-stream rate-limited failure logging with recovery notices is implemented (section 13, issue #68).
- Full benchmark matrix (mass-expiry drain p99, maxlen sensitivity) with CI regression gates is implemented (section 11, issue #70): `bench/run.sh` runs the scenarios, `bench/gate.sh` gates the ratios, and the nightly bench workflow executes both.
