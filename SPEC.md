...
- Custom `@eventstream` ACL category is implemented (section 8, issue #69): `EVENTSTREAM.STATS` and `EVENTSTREAM.STREAMS` are tagged into it via `RM_AddACLCategory`/`RM_SetCommandACLCategories` where the API exists (Redis 7.4+, Valkey builds that expose it), and it degrades to a clean skip with one load-time notice on Redis 7.2/7.3 (the null-pointer gate) — the commands stay individually grantable everywhere. Per-stream rate-limited failure logging with recovery notices is implemented (section 13, issue #68).
