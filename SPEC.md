...
- **Per source node in cluster mode.** Each node's module instance has its own counter, so `seq` orders entries per node, not across nodes. Cross-node same-millisecond ties still need an application timestamp (section 9, docs/src/topologies.md).
...
- Cluster support: the slot-pinned per-node hashtag design shipped in v0.2.0 as `eventstream.cluster-streams per-node` (section 10), including per-node discovery and reshard handling. The user-facing topology and consumer requirements are documented in [docs/src/topologies.md](https://github.com/joshrotenberg/redis-event-stream-module/blob/main/docs/src/topologies.md); this specification retains the as-built mechanism and design rationale.
